...
<#
    ...
    Or straight from GitHub:     irm https://raw.githubusercontent.com/Youssef-Eltohamy/kblangfix/main/install.ps1 | iex

    Everything it does is local: installs AutoHotkey v2 (via winget) if missing,
    copies the script to %LOCALAPPDATA%\KbLangFix, adds a Startup shortcut, runs it.
#>
[CmdletBinding()]
param(
    [string]$Dest    = "$env:LOCALAPPDATA\KbLangFix",
    [string]$RepoRaw = "https://raw.githubusercontent.com/Youssef-Eltohamy/kblangfix/main",
    [switch]$NoStartup,
    [switch]$NoLaunch
)
...
$files  = 'KbLangFix.ahk', 'KbLangFix.test.ahk', 'settings.ini.example', 'assets/kblangfix.ico'
$srcDir = $PSScriptRoot
...
# --- 4. run at login ---------------------------------------------------
if (-not $NoStartup) {
    $lnk = Join-Path ([Environment]::GetFolderPath('Startup')) 'KbLangFix.lnk'
    $s   = (New-Object -ComObject WScript.Shell).CreateShortcut($lnk)
    $s.TargetPath       = $ahk
    $s.Arguments        = '"' + (Join-Path $Dest 'KbLangFix.ahk') + '"'
    $s.WorkingDirectory = $Dest
    $s.IconLocation     = (Join-Path $Dest 'assets\kblangfix.ico')
    $s.Description      = 'KbLangFix - fix wrong-keyboard-layout text'
    $s.Save()
...
# --- 5. launch ---------------------------------------------------------
if (-not $NoLaunch) {
    Start-Process $ahk -ArgumentList ('"' + (Join-Path $Dest 'KbLangFix.ahk') + '"')
    Write-Host ""
